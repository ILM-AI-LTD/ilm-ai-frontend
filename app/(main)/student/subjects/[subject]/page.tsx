...
const page = ({ params }: PageProps) => {
  ...
  const mergedData = data?.data?.topics
    ? mergeProgress(rawData, data.data.topics)
    : rawData;
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-20 md:w-max">
      <div className="order-2 lg:order-1">
        <ChapterList
          topics={mergedData.topics}
          subject={subject as "physics" | "chemistry" | "math" | "biology"}
        />
      </div>

      <div className="order-1 lg:order-2">
        <div className="flex flex-col md:flex-row lg:flex-col items-center md:items-start gap-4 lg:sticky lg:top-30">
          <NavigationCard
            subject={subject as "physics" | "chemistry" | "math" | "biology"}
          />
