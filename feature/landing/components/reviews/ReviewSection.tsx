...
export default function ReviewSection() {
  ...
  return (
    <section className=" w-full flex flex-col items-center justify-center py-20 px-6 md:px-10 2xl:px-[135px] gap-8">
      ...
      <div className="relative w-[90%] overflow-hidden rounded-3xl mask-l-from-70% mask-r-from-70%">
        <div className="flex gap-8 animate-marquee-reverse will-change-transform mb-8">
          {looped.map((t, i) => (
            <div
              key={i}
              className={`flex-shrink-0 flex flex-col justify-between p-6  bg-background rounded-2xl border-1 border-border-color text-white ${
                i % 2 !== 0 ? "w-80" : "w-96"
              }`}
            >
      ...
      <CustomButton
        className={`btn-hover inline-flex h-12 w-1/4 items-center justify-center rounded-full  bg-gradient-to-b from-[#1D2840] to-[#000000] text-white font-bold hover:bg-[#007AAC] hover:text-white border-1 border-[#373C4E] shadow-[0px_6px_0px_0px_#373C4E]`}
