...
const MarkdownRenderer: React.FC<MarkdownRendererProps> = React.memo(({ content, isStreaming = false }) => {
  ...
  const markdownComponents = useMemo(() => ({
    ...
    p: ({ children }: { children: React.ReactNode }) => <p className="text-base text-foreground mb-2 leading-relaxed">{children}</p>,
    ...
    li: ({ children }: { children: React.ReactNode }) => <li className="text-foreground text-base">{children}</li>,
