...
const MCQQuestionComponent: React.FC<MCQQuestionComponentProps> = ({
    ...
    const getOptionStyle = (option: any) => {
        if (!evaluationResult) {
            ...
            return `rounded-lg border p-4 cursor-pointer flex transition-colors duration-200 ${isSelected ? 'border-primary bg-primary/10' : 'hover:bg-primary/20'
                }`;
        } else {
            const isSelected = selectedOption === option.value;
    ...
    const getOptionNumber = (option: any) => {
        if (!evaluationResult) {
            const isSelected = selectedOption === option.value;
            return (
                <div className={`border w-10 h-10 rounded-sm mr-4 flex justify-center items-center font-semibold ${isSelected ? "border-primary" : ""
                    }`}>
                    {option.value}
                </div>
            );
